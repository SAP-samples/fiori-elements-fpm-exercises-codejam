...
We manually added two main blocks of annotations for the bookshop service to our SAP Fiori elements application. The first two of them are relevant for the list report page of the application. The first code block contains simple labels for the fields of the entity. The second block contains annotations relevant for the list report page. Let's go through them one by one to make sure we really understand them:
